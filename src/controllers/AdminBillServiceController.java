...
public class AdminBillServiceController {
		...
		try (Connection connection = SqliteConnection.Connector();
			...
			while (resultSet.next()) {
				...
				for (Room room : bookedRooms) {
					System.out.println(
							"room info: " + room.getRoomID() + " " + room.getRoomType() + " " + room.getPrice());
				}
				...
				for (Map.Entry<RoomTypeName, Integer> entry : roomTypeCount.entrySet()) {
					...
					roomTypesInfo.append(roomType).append(": ").append(count).append(" room(s) - $").append(price)
							.append(" per room\n");
				}
	...
	void handleCheckOut(ActionEvent event) {
		...
		try (Connection connection = SqliteConnection.Connector()) {
			...
			String deleteReservationRoomsQuery = "DELETE FROM ReservationRooms WHERE reservationID = ?";
			...
			try (PreparedStatement ps3 = connection.prepareStatement(deleteReservationRoomsQuery)) {
				ps3.setInt(1, bookID);
				ps3.executeUpdate();
			}
